...
public sealed partial class CE14WorkbenchWindow : DefaultWindow
{
    ...
    private CE14ClientSkillSystem _skill;

    public event Action<CE14WorkbenchUiRecipesEntry>? OnCraft;
    ...
    private Dictionary<LocId, List<CE14WorkbenchUiRecipesEntry>> _categories = new();
    private List<CE14WorkbenchUiRecipesEntry> _uncategorized = new();

    private CE14WorkbenchUiRecipesState? _cachedState;
    private CE14WorkbenchUiRecipesEntry? _selectedEntry;
    private string _searchFilter = string.Empty;

    private ISawmill Sawmill { get; init; }

    public CE14WorkbenchWindow()
    {
        RobustXamlLoader.Load(this);
        IoCManager.InjectDependencies(this);

        _skill = _entManager.System<CE14ClientSkillSystem>();

        Sawmill = _log.GetSawmill("CE14_workbench_window");
    ...
    private void AddRecipeListToGrid(List<CE14WorkbenchUiRecipesEntry> category, GridContainer gridContainer)
    {
        foreach (var entry in category)
        {
            ...
            var control = new CE14WorkbenchRecipeControl(entry);
    ...
    public void UpdateState(CE14WorkbenchUiRecipesState recipesState)
    {
        ...
        OptionCategories.AddItem(Loc.GetString("CE14-recipe-category-all"), AllCategoryId);
        ...
        var sortedRecipes = recipesState.Recipes
            ...
            .ThenByDescending(e =>
            {
                if (!_prototype.TryIndex(e.ProtoId, out CE14WorkbenchRecipePrototype? recipe))
                    return 0;
            ...
            {
                if (!_prototype.TryIndex(e.ProtoId, out CE14WorkbenchRecipePrototype? recipe) ||
                    !_prototype.TryIndex(recipe.Category, out CE14WorkbenchRecipeCategoryPrototype? category))
                    return string.Empty;
                return category.ID;
            });
        ...
        foreach (var entry in sortedRecipes)
        {
            ...
            {
                entries = new List<CE14WorkbenchUiRecipesEntry>();
        ...
        var sortedCategories = _categories
            .OrderByDescending(c =>
            {
                var categoryProto = _prototype.EnumeratePrototypes<CE14WorkbenchRecipeCategoryPrototype>()
    ...
    private bool ProcessSearchFilter(CE14WorkbenchUiRecipesEntry entry, CE14WorkbenchRecipePrototype indexedEntry)
    {
    ...
    private bool ProcessSearchCategoryFilter(CE14WorkbenchRecipePrototype indexedEntry)
    {
    ...
    private void RecipeSelect(CE14WorkbenchUiRecipesState recipesState)
    {
    ...
    private void RecipeSelect(CE14WorkbenchUiRecipesEntry cachedEntry)
    {
        if (_cachedState is null)
            return;
    ...
    private void RecipeSelect(CE14WorkbenchUiRecipesEntry entry, CE14WorkbenchRecipePrototype recipe)
    {
