...
public sealed partial class CE14DemiplaneMapWindow : DefaultWindow
{
    [Dependency] private readonly IPrototypeManager _prototype = default!;
    [Dependency] private readonly ILogManager _log = default!;
    [Dependency] private readonly IClientAdminManager _admin = default!;

    private CE14DemiplaneMapUiState? _cachedState;

    private ISawmill Sawmill { get; init; }

    public CE14DemiplaneMapWindow()
    {
        RobustXamlLoader.Load(this);
        IoCManager.InjectDependencies(this);

        Sawmill = _log.GetSawmill("CE14_demiplane_map_window");
    ...
    public void UpdateState(CE14DemiplaneMapUiState state)
    {
        _cachedState = state;
        ...
        foreach (var node in state.Nodes)
        {
            if (node.Value.Start)
            {
                var startElement = new CE14NodeTreeElement(
                    nodeKey: node.Key.ToString(),
                    gained: true,
                    active: false,
                    node.Value.UiPosition * 100,
                    icon: new SpriteSpecifier.Rsi(new ResPath("_CE14/Interface/NodeTree/demiplane_map.rsi"), "center"));
            ...
            else
            {
                ...
                var treeElement = new CE14NodeTreeElement(
                    nodeKey: node.Key.ToString(),
        ...
        GraphControl.UpdateState(
            new CE14NodeTreeUiState(
                nodeTreeElements,
                edges: edges,
                frameIcon: new SpriteSpecifier.Rsi(new ResPath("/Textures/_CE14/Interface/NodeTree/demiplane_map.rsi"),
                    "frame"),
                hoveredIcon: new SpriteSpecifier.Rsi(
                    new ResPath("/Textures/_CE14/Interface/NodeTree/demiplane_map.rsi"),
                    "hovered"),
    ...
    private void SelectNode(CE14DemiplaneMapNode? node)
    {
